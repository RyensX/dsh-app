...
import { liteActions } from './view'

const root = document.querySelector<HTMLElement>('#app')
if (!root) throw new Error('missing #app')

let info: BootstrapInfo | null = null
let status: LaunchStatus = { state: 'starting', message: 'Checking the Lite runtime requirements...' }

const draw = (): void => render(root, info, status, {
  edition: 'lite',
  retry: begin,
  openDataDirectory: revealDataDirectory,
  extra: liteActions(chooseNode),
})
...
async function chooseNode(): Promise<void> {
  const selected = await open({
    title: 'Choose a Node executable',
    multiple: false,
    directory: false,
  })
  if (typeof selected !== 'string') return
  status = { state: 'starting', message: 'Validating the selected Node executable...' }
  draw()
  try {
    await invoke('set_lite_node_path', { path: selected })
    await begin()
  } catch (error) {
    status = { state: 'failed', error: toLaunchError(error, 'The selected Node could not be saved') }
    draw()
  }
}

async function begin(): Promise<void> {
  status = { state: 'starting', message: 'Checking the Lite runtime requirements...' }
  draw()
  try {
